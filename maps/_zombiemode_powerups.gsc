...
init_powerups()
{
	...
	add_zombie_powerup( "free_perk", "zombie_pickup_perk_bottle", &"ZOMBIE_POWERUP_FREE_PERK" );
	
	
	add_zombie_powerup( "tesla", "zombie_pickup_minigun", &"ZOMBIE_POWERUP_MINIGUN", level._effect[ "minigun_pickup" ] ); 
...
specific_powerup_drop( powerup_name, drop_spot )
{
	...
	{
		if ( "tesla" == powerup_name && tesla_powerup_active() )
		{
			
			powerup_name = "minigun";
		}
	...
	while (isdefined(self))
	{
		...
		for (i = 0; i < players.size; i++)
		{
			...
			if (distance (players[i].origin, self.origin) < 64)
			{
				...
				else if( IsDefined( self.powerup_name ) && self.powerup_name == "tesla" )
				{
					PlayFX( level._effect[ "powerup_grabbed_solo" ], self.origin );
				}
				else
