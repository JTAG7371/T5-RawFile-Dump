...
health_think()
{
	...
	for (;;)
	{
		...
		slideVelocity = length( velocity - ( vector_scale( collisionNormal, impactVelocity ) ) );
		...
		directionOfImpact = vector_scale( collisionNormal, -1 );
...
borderwall_method1()
{
	...
	for(;;)
	{
		...
		movetospot = level.playervehicle.origin + vector_scale( normalvec, 2000 );
...
missileIndicator( eMissile )
{
	...
	for (;;)
	{
		...
		backvec = vector_scale( forwardvec, -1 );
		rightvec = anglestoright( level.player.angles );
		leftvec = vector_scale( rightVec, -1 );
